    ...
    <ion-content>
      <ion-list>
        ...
        <ion-item>
          ...
          <ion-input
            slot="end"
            class="range-value-input price-value-input"
        ...
        <ion-item>
          ...
          >
            ...
            <ion-select-option
              v-for="s in woodSpeciesList"
              :key="s"
              :value="s"
              >{{ s }}</ion-select-option
            >
            <ion-select-option :value="ADD_NEW_VALUE"
              >Add New...</ion-select-option
            >
          </ion-select>
        </ion-item>
      </ion-list>
      ...
      <ion-list inset>
        <ion-item color="primary" class="totals-bar">
          ...
          <ion-label slot="end" class="totals-end">{{
...
function updateThickness(v: any) {
  thickness.value = clamp(Number(v), 5, 400);
}
function updateLength(v: any) {
  length.value = clamp(Number(v), 100, 10000);
}
function updateQuantity(v: any) {
  quantity.value = clamp(Number(v), 1, Number(settingsMaxQuantity.value));
}
function updatePrice(v: any) {
...
const totalPrice = computed(() => {
  return (
    Math.round(Number(totalM3.value) * Number(pricePerMeter3.value) * 100) / 100
  );
});
...
const grandTotalM3 = computed(() =>
  lumberItems.value.reduce((sum, item) => sum + item.totalBft, 0),
);
const grandTotalPrice = computed(() =>
  lumberItems.value.reduce((sum, item) => sum + item.totalPrice, 0),
);
...
function onSendEmail() {
  logEvent("send_email", { type: "lumber_list_metric" });
  ...
  lumberItems.value.forEach((item, idx) => {
    ...
    text +=
      ...
      " mm</td>" +
      "<td>" +
      formatM3(item.pieceBft) +
      "</td>" +
      "<td>" +
      formatM3(itemTotalBft) +
      "</td>" +
      "<td>" +
      formatMoney(round(item.pricePerMeter3, 5)) +
      "</td>" +
      "<td>" +
      formatMoney(round(item.piecePrice, 2)) +
      "</td>" +
      "<td>" +
      formatMoney(itemTotalPrice) +
      "</td></tr>\n";
