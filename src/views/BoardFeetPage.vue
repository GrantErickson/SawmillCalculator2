...
function updatePricePer1000(v: any) {
  pricePer1000.value = Number(v) || 0;
}
function updateWoodType(v: any) {
  woodType.value = v;
}
...
const totalPrice = computed(() => {
  return (
    Math.round(
      ((Number(totalBft.value) * Number(pricePer1000.value)) / 1000) * 100,
    ) / 100
  );
});
...
function onSendEmail() {
  logEvent("send_email", { type: "lumber_list_us" });
  ...
  lumberItems.value.forEach((item, idx) => {
    ...
    text +=
      ...
      "<td>" +
      item.thickness +
      (item.woodType == WoodTypes.Hardwood ? "/4" : "") +
      '"</td>' +
      "<td>" +
      item.width +
      '"</td>' +
      ...
      "'</td>" +
      "<td>" +
      formatBft2(item.pieceBft) +
      "</td>" +
      "<td>" +
      formatBft2(itemTotalBft) +
      "</td>" +
      "<td>" +
      formatMoney(round(item.pricePer1000, 2)) +
      "</td>" +
      "<td>" +
