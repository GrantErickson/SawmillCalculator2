...
$(document).delegate("#pageCutList", "pageinit", function () {
	...
	var CutListModel = function () {
		...
		self.kerf.subscribe(function (value: string) {
			window.localStorage.setItem("Kerf", value);
			$("#kerf").selectmenu("refresh");
			self.calculate();
		});
		...
		self.total.subscribe(function (value: string) {
			window.localStorage.setItem("Total", value);
			self.calculate();
		});
		self.flitch.subscribe(function (value: string) {
