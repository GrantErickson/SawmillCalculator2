...
export const moneySymbolLocation = ref(
  localStorage.getItem("moneySymbolLocation") || "before",
);
  ...
  if (stored) {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0)
        return parsed.sort((a: string, b: string) => a.localeCompare(b));
    } catch {
...
export function addSpecies(name: string): boolean {
  ...
  if (
    woodSpeciesList.value.some((s) => s.toLowerCase() === trimmed.toLowerCase())
  )
    return false;
...
watch(moneySymbolLocation, (v) =>
  localStorage.setItem("moneySymbolLocation", v),
);
watch(
  woodSpeciesList,
  (v) => localStorage.setItem("woodSpeciesList", JSON.stringify(v)),
  { deep: true },
);
